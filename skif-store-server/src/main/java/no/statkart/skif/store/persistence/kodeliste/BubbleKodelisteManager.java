 ...
 * Lokalisering av en {@link BubbleKode kode} eller {@link BubbleKodeliste kodeliste}
 ...
 * @since 0.6
 */
public class BubbleKodelisteManager {
     ...
     */
    private Map<BubbleIdInterface<?>, BubbleObjectInterface> nonLocalizedStaticCache = new HashMap<BubbleIdInterface<?>, BubbleObjectInterface>();


    private volatile Collection<? extends BubbleKodelisteId<?>> kodelisteIds;

    private volatile Collection<? extends BubbleKodeId<?>> kodeIds;

    /**
     * Alle koder og kodelister (inkl enum koder og kodelister)
     */
    private volatile Map<BubbleIdInterface<?>, BubbleObjectInterface> nonLocalizedcache;
     ...
     */
    private volatile Map<BubbleIdInterface<?>, BubbleObjectInterface> localizedCache_n;
    ...
    public synchronized void installStatic(Class<? extends EnumBubbleKodeId<? extends EnumBubbleKode>> enumKodeIdClass) {
        EnumBubbleKodeSupport kodeSupport = EnumBubbleKodeSupport.getKodeSupport(enumKodeIdClass);
        EnumBubbleKodeliste kodeliste = kodeSupport.getNonLocalizedKodeliste();
        Collection<EnumBubbleKode> koder = kodeSupport.getNonLocalizedKoder();
        installStatic(kodeliste, koder);
    }

    public synchronized void installStatic(BubbleKodeliste kodeliste, Collection<? extends BubbleKode> koder) {
        BubbleObjectInterface existingKodeliste = nonLocalizedStaticCache.put(kodeliste.getId(), kodeliste);
        ...
        for (BubbleKode kode : koder) {
            BubbleObjectInterface existingKode = nonLocalizedStaticCache.put(kode.getId(), kode);
    ...
    public synchronized void installStatic(Collection<? extends BubbleKodeliste> kodelister, Collection<? extends BubbleKode> koder) {
        for (BubbleKodeliste kodeliste : kodelister) {
            BubbleObjectInterface existingKodeliste = nonLocalizedStaticCache.put(kodeliste.getId(), kodeliste);
            if (existingKodeliste != null) {
                throw new ImplementationException("Kodeliste med samme id allerede installert: eksisterende=" + existingKodeliste + " ny=" + kodeliste);
            }
    ...
    public synchronized void updateDynamic(Collection<? extends BubbleKodeliste> kodelister, Collection<? extends BubbleKode> koder) {
        int cacheSize = nonLocalizedStaticCache.size() + kodelister.size() + koder.size();
        Map<BubbleIdInterface<?>, BubbleObjectInterface> cache = new HashMap<BubbleIdInterface<?>, BubbleObjectInterface>(cacheSize);
        Collection<BubbleKodeId<?>> kodeIds = new ArrayList<BubbleKodeId<?>>(koder.size());
        Collection<BubbleKodelisteId<?>> kodelisteIds = new ArrayList<BubbleKodelisteId<?>>(kodelister.size());

        for (Map.Entry<BubbleIdInterface<?>, BubbleObjectInterface> entry : nonLocalizedStaticCache.entrySet()) {
            cache.put(entry.getKey(), entry.getValue());
            if (entry.getKey() instanceof BubbleKodelisteId<?>) {
               kodelisteIds.add((BubbleKodelisteId<?>)entry.getKey());
            } else {
                kodeIds.add((BubbleKodeId<?>)entry.getKey()); 
            }
        }
     ...
     * @return
     */
    public BubbleObject get(BubbleIdInterface<?> bubbleId, Locale lokale) {
    ...
    private synchronized Map<BubbleId<?>, BubbleObject> initializeLocalizedCache(Locale lokale) {
        if (localizedCache_b == null) {
            if (nonLocalizedcache==null) {
               nonLocalizedcache = new HashMap<BubbleIdInterface<?>, BubbleObjectInterface>(nonLocalizedStaticCache);
            }
            Map<BubbleId<?>, BubbleObject> localizedCache = new HashMap<BubbleId<?>, BubbleObject>(nonLocalizedcache.size());
            for (Map.Entry<BubbleIdInterface<?>, BubbleObjectInterface> e : nonLocalizedcache.entrySet()) {
                BubbleObjectInterface copy = CopyHelper.copy(e.getValue());
    ...
    private BubbleObjectInterface localizeObject(Locale lokale, BubbleObjectInterface bubbleObject) {
        // TODO: Bruke lokale
        if (bubbleObject instanceof EnumBubbleKode) {
            EnumBubbleKode bubbleKode = (EnumBubbleKode) bubbleObject;
            // TODO: Lokaliser!
            String lokalisertBeskrivelse = bubbleKode.getBeskrivelsesKey();
            bubbleKode.setBeskrivelse(lokalisertBeskrivelse);
        } else if (bubbleObject instanceof DbBubbleKode) {
            DbBubbleKode bubbleKode = (DbBubbleKode) bubbleObject;
            String lokalisertBeskrivelse = bubbleKode.getLokalisertBeskrivelse().get("b");
            bubbleKode.setBeskrivelse(lokalisertBeskrivelse);
        } else if (bubbleObject instanceof EnumBubbleKodeliste) {
            EnumBubbleKodeliste kodeliste = (EnumBubbleKodeliste) bubbleObject;
            // TODO: Lokaliser!
            String lokalisertBeskrivelse = kodeliste.getBeskrivelsesKey();
            kodeliste.setBeskrivelse(lokalisertBeskrivelse);
        } else {
            DbBubbleKodeliste kodeliste = (DbBubbleKodeliste) bubbleObject;
    ...
    public synchronized BubbleKodelisteTransfer getKodelisteTransfer(Locale locale) {
        return new BubbleKodelisteTransfer(kodeIds, kodelisteIds, getAllKodelisterAndKoder(locale));
    }
}
