...
/**
 * Class for accessing BubbleId property values on objects via a getter/setter pair, which may be
 * nonpublic. The implementation is based on the default implementation  {@link
 * org.hibernate.property.BasicPropertyAccessor}, but has been modified to produce a getter/setter pair
 * with postfix "Id" for a given property. E.g. a mapping property named "user" would produce a
 * getUserId/setUserId pair.
 *
 * @author Henrik Fredholm
 */
public class BubbleRefAccessor implements PropertyAccessor {

   private static final Logger log = LoggerFactory.getLogger(BubbleRefAccessor.class);

   public static final class BubbleRefSetter implements Setter {
      private Class clazz;
      private final Method method;
      private final String propertyName;

      private BubbleRefSetter(Class clazz, Method method, String propertyName) {
         this.clazz = clazz;
         this.method = method;
   ...
   public static final class BubbleRefGetter implements Getter {
      private Class clazz;
      private final Method method;
      private final String propertyName;

      private BubbleRefGetter(Class clazz, Method method, String propertyName) {
         this.clazz = clazz;
         this.method = method;
         this.propertyName = propertyName;
      }
...
// Add "Id" postfix to propertyname.
      propertyName += "Id";
      BubbleRefSetter result = getSetterOrNull(theClass, propertyName);
      if (result == null)
         throw new PropertyNotFoundException("Could not find a setter for property " + propertyName + " in class " + theClass.getName());
      return result;
   }

   private static BubbleRefSetter getSetterOrNull(Class theClass, String propertyName) {
      ...
      if (method != null) {
         ...
         return new BubbleRefSetter(theClass, method, propertyName);
      } else {
         BubbleRefSetter setter = getSetterOrNull(theClass.getSuperclass(), propertyName);
         if (setter == null) {
   ...
   private static Method setterMethod(Class theClass, String propertyName) {
      ...
      BubbleRefGetter getter = getGetterOrNull(theClass, propertyName);
   ...
   public Getter getGetter(Class theClass, String propertyName) throws PropertyNotFoundException {
      propertyName += "Id";
      BubbleRefGetter result = getGetterOrNull(theClass, propertyName);
      if (result == null)
   ...
   private static BubbleRefGetter getGetterOrNull(Class theClass, String propertyName) {
      ...
      if (method != null) {
         if (!ReflectHelper.isPublic(theClass, method)) method.setAccessible(true);
         return new BubbleRefGetter(theClass, method, propertyName);
      } else {
         BubbleRefGetter getter = getGetterOrNull(theClass.getSuperclass(), propertyName);
         if (getter == null) {
            Class[] interfaces = theClass.getInterfaces();
