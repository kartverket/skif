...
public class ActionQueue {
    ...
    private class InsertActionSorter {
        ...
        private Integer findBatchNumber(
            ...
            // MODIFIED SKIF-428 Start
            // The new findBatchNumber method also look into composite components when calculation what batch number to use
            // Composite components may have a back pointer to owner, but this will be null during the insert.
            Integer foundBatchNumerForType = findBatchNumber(latestBatchNumberForType, propertyValues, propertyTypes);
            if (!foundBatchNumerForType.equals(latestBatchNumberForType)) {
                latestBatches.put(entityName, foundBatchNumerForType);
            }
            return foundBatchNumerForType;
            // MODIFIED SKIF-428 End
        }

        // MODIFIED SKIF-428 Start
        private Integer findBatchNumber(Integer latestBatchNumberForType, Object[] propertyValues, Type[] propertyTypes) {
