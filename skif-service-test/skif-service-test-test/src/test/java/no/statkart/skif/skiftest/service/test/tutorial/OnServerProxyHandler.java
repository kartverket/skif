...
public class OnServerProxyHandler<S> extends ChainedProxyHandler<S> {
    ...
    protected Object invokeMethod(Object proxy, Method method, Object[] args) throws Throwable {
        try {
            System.out.println("OnServerProxyHandler<" + type.getRawType().getSimpleName() + ">." + method.getName() + " - Begin");
            return chained.invoke(proxy, method, args);
        } finally {
            System.out.println("OnServerProxyHandler<" + type.getRawType().getSimpleName() + ">." + method.getName() + " - End");
