...
public class SkifTestMappingDefaultTypeMapperD2WTest {
    ...
    public void testMapTestC(){
        ...
        assertEquals(target.getInts().getItem(), Ints.asList(source.getInts()));
        assertEquals(target.getStrings().getItem().toArray(), source.getStrings());
        for(int i = 0; i < source.getAs().length;i++) {
            assertEquals(target.getAs().getItem().get(i).getText(), source.getAs()[i].getText());
        }
    }
