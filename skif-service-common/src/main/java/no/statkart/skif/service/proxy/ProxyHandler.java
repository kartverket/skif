...
/**
 * Abstrakt superklasse for {@code ProxyHandler}s som kan inngå i en kjede av {@code ProxyHandler}s, også kallet
 * en {@code ServiceChain}. Formålet med en {@code ServiceChain} er å utfører operasjoner i forkant eller etterkant
 * av servicekallet. I en {@code ServiceChain} vil alle {@code ProxyHandler}-ledd på nær det siste normalt være av subtypen
 * {@link ChainedProxyHandler} og  siste ledd vil normalt vil være av subtypen {@link TerminatingProxyHandler}.
 * Dette er dog ikke noe formelt krav. En {@code ServiceChain} representeres ikke av et egen klasse men ved den
 * første {@code ProxyHandler} i kjeden. Det er mulig å utvide en {@ServiceChain} ved å legge på flere
 * {@code ProxyHandlere} foran en eksisterende {@code ServiceChain}, men det er ingen funksjonalitet for å
 * legge på {code ProxyHandler}s bak en eksisterende {@code ServiceChain}.
 * <p/>
