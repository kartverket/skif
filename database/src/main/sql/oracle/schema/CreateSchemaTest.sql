...
CREATE TABLE BUBBLE_WITH_ENT_MULTKOB (
    ID NUMBER(19,0) NOT NULL,
    CONSTRAINT PK_BUBBLE_WITH_ENT_MULTKOB PRIMARY KEY (ID)
);

--todo: forbedret navn
CREATE TABLE ENT_IN_MULTIKOB_KOBLING (
    ID NUMBER(19,0) NOT NULL,
    OWNERID NUMBER(19,0) constraint ENT_IN_MULTIKOB_OWNERID_NULL NOT NULL INITIALLY DEFERRED,
    ROLLE VARCHAR2(255 CHAR) NOT NULL,
    TEKST VARCHAR2(255 CHAR) NOT NULL,
    CONSTRAINT PK_ENT_IN_MULTIKOB_KOBLING PRIMARY KEY (ID)
);

ALTER TABLE ENT_IN_MULTIKOB_KOBLING ADD (
  CONSTRAINT FK_OWNERID_1 FOREIGN KEY (OWNERID) REFERENCES BUBBLE_WITH_ENT_MULTKOB(ID)
);


-- Tabeller for subtyped entity component testing

create table BubbleWithSubtypedEntityComponent (
   id number(19,0) not null,
   subtypedComponentId number(19,0),
   primary key (id)
);

create table SubtypedEntityComponent (
   id number(19,0) not null,
   ownerId number(19,0) constraint SubtypedEComponent_ownerId_null not null initially deferred,
   class varchar2(255) not null,
   nr number(10,0),
   primary key (id)
);
...
alter table X1AAForX1CCMany add constraint FK_X1AAForX1CCMany_ownerId foreign key (ownerId) references X1AA;
alter table X1AAForX1CCMany add constraint FK_X1AAForX1CCMany_childId foreign key (childId) references X1CCMany;
...
alter table X2AAForX2CCMany add constraint FK_X2AAForX2CCMany_ownerId foreign key (ownerId) references X2EntityComponentOne;
alter table X2AAForX2CCMany add constraint FK_X2AAForX2CCMany_childId foreign key (childId) references X2CCMany;
