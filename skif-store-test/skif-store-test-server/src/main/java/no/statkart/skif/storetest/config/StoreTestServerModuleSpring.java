...
import no.statkart.skif.store.service.ejb.EJBResourceProxyHandlerForHibernateWithLocksSpring;
 ...
 * @since 2.0
 */
public class StoreTestServerModuleSpring extends SkifModule {
    public StoreTestServerModuleSpring(Configuration configuration) {
        super(configuration);
    }
    ...
    protected ModuleStrategyFactory defineDefaultModuleStrategyFactory() {
        ...
        prototype.getStrategy(ServiceMode.JEE).setEjbServiceChainFactorySpecification(new EJBServiceChainFactorySpecification(EJBResourceProxyHandlerForHibernateWithLocksSpring.class));
...
//                .addBubble(BarFoos.class)
    ...
    @Provides
    @Singleton
    HibernateSessionFactoryManagerBundle provideHibernateSessionFactoryManagerBundle(Provider<IdService> idServiceProvider, BubbleModelConfiguration bubbleModelConfiguration) {
        ...
        if (moduleConfiguration.getServiceMode() == ServiceMode.SINGLE_VM) {
            throw new ImplementationException("ServiceMode.SINGLE_VM not support for Spring");
        } else {
            // Spring
            ...
            hibernatePropertiesCurrent.setProperty(Environment.CONNECTION_PROVIDER, no.statkart.skif.persistence.hibernate.PoolConnectionProviderSpring.class.getName());
            hibernatePropertiesCurrent.put(Environment.DATASOURCE, applicationContext.getBean(DataSource.class));

            hibernatePropertiesOld = ConfigurationConverter.getProperties(hibernatePropertiesConfiguration);
            hibernatePropertiesOld.setProperty(Environment.CONNECTION_PROVIDER, no.statkart.skif.persistence.hibernate.PoolConnectionProviderSpring.class.getName());
            hibernatePropertiesOld.put(Environment.DATASOURCE, applicationContext.getBean("dataSourceOld", DataSource.class));
...
//            hibernatePropertiesOld.setProperty(Environment.DATASOURCE, datasourceOld);
    ...
    DataSource provideConnectionPool() {
        if (moduleConfiguration.getServiceMode() == ServiceMode.SINGLE_VM || moduleConfiguration.getServiceMode() == ServiceMode.SINGLE_VM_XML) {
            throw new ImplementationException("SINGLE_VM mode not supported for Spring");
        } else {
            Configuration configuration = moduleConfiguration.getConfiguration();
            ApplicationContext applicationContext = (ApplicationContext) configuration.getProperty("spring.application.context");
    ...
    ResourceManager createResourceManagerForConnectionOnlyStrategy(Provider<DataSource> dataSourceProvider) {
        ...
        if (moduleConfiguration.getServiceMode() == ServiceMode.SINGLE_VM || moduleConfiguration.getServiceMode() == ServiceMode.SINGLE_VM_XML) {
            ...
            // Spring bruker ikke DataSource lookup via JDNI
            connectionManager = new ConnectionManagerUsingFactory(new ConnectionFactoryUsingPoolSpring(dataSourceProvider.get(), false, SnapshotVersion.CURRENT, false));
        }
